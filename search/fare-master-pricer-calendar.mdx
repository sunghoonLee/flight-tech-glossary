---
title: "Fare_MasterPricerCalendar"
description: "Amadeus의 날짜 유연 검색 API인 FareMasterPricerCalendar의 기술 용어를 정리한 문서입니다. 기준 날짜 전후 범위의 최저가를 달력 형태로 탐색한다."
---
...
<Info title="WBS Integration Flow Step 1">
이 API는 [WBS Integration Flow](/amadeus-wbs-integration-flow)의 **검색 단계 Step 1**에 해당합니다.
</Info>
---
...
Amadeus GDS가 제공하는 **달력 기반 최저가 탐색 API**. [Fare_MasterPricerTravelBoardSearch](/search/master-pricer-travelboard-search)(MPTBS)와 동일한 메시지 구조를 사용하되, **날짜 범위(dayInterval)와 여행 기간 유연성(tripInterval)**을 지정하여 전후 날짜별 최저가를 달력 형태로 반환한다.
...
| 다음 단계 | [Fare_MasterPricerTravelBoardSearch](/search/master-pricer-travelboard-search) |
...
<CardGroup cols={2}>
  <Card title="WBS Integration Flow - Step 1" href="/amadeus-wbs-integration-flow" icon="link" />
  <Card title="Fare_MasterPricerTravelBoardSearch 용어집" href="/search/master-pricer-travelboard-search" icon="link" />
</CardGroup>
